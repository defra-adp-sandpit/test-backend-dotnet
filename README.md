# test-backend-dotnet
test-backend-dotnet Dotnet service.
...
| ApplicationInsights__CloudRole          | Role used for filtering metrics    | no       |           |       | Set to `test-backend-dotnet-local` in docker compose files    |
...
`docker-compose -p test-backend-dotnet-test -f docker-compose.yaml -f docker-compose.test.yaml up`
...
`docker-compose -p test-backend-dotnet-test -f docker-compose.yaml -f docker-compose.test.watch.yaml up`
...
```
variableGroups:
    - test-backend-dotnet-snd1
    - test-backend-dotnet-snd2
    - test-backend-dotnet-snd3
variables:
    - test-backend-dotnet-APPLICATION-SECRET
```
